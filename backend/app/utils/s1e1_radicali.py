...
class RadicalExpressionGenerator:
    ...
    def generate_type13(self) -> Dict:
            ...
            steps = [
                f"Let's solve this step by step using the square of binomials formula: (x + y)² = x² + 2xy + y²",
                f"First part ({a} + √{b})²: {a}² + 2·{a}·√{b} + (√{b})²",
                f"Simplify first part: {a**2} + {2*a}√{b} + {b}",
                f"Second part (1 - {a}√{b})²: 1² + 2·1·(-{a}√{b}) + ({-a}√{b})²",
                f"Simplify second part: 1 - {2*a}√{b} + {a**2}·{b}",
                f"Add both parts together and combine like terms",
...
if __name__ == "__main__":
    ...
    for exercise_type in range(1, 2):
        try:
            for i in range(100):
